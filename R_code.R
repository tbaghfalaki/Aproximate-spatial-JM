...
Spaghetti.Plot(
  Dataset = Data, Outcome = cd4, Id = patient, Time = timecd4,
...
linear.covariate <- data.frame(
  mu = as.factor(c(rep(1, n1), rep(2, n2))),
...
formula <- Y~ mu - 1 + l.Age + l.Gender + l.POI + l.SS + s.Age + s.Gender + s.POI + f(U11, model = "iid2d", n = 2 * n2) +
  ...
  f(U22, copy = "U11", fixed = FALSE) + f(ZONE_CODE,
    model = "besag", # spatial effect: ZONE_CODE is a numeric identifier for each area in the lattice  (does not work with factors)
    graph = Lattice.adj
  )
